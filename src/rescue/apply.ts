 ...
 * branches AND rescue adoption, adopt.ts). A terminal run carries no quota
 * checkpoint (finalize cleared it), and `terminal_reason` must be dropped: the
 * schema forbids a `running` run that still carries one, so a reopen that kept it
 * would fail re-validation.
 */
export function reopenFields(reopen: boolean): Partial<RunState> {
